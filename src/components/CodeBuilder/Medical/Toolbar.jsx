...
const CustomToolbarSelect = observer(({ classes, selectedRows, setSelectedRows }) => {
    ...
    function inverseSelection() {
        const nextSelectedRows = medicalStore.masterCodeList?.reduce(
    ...
    function removeSelected() {
        medicalStore.removeMasterCodes(selectedRows);
    ...
    function addSelected() {
        ...
        dataIndexesToAdd
            .filter(index => {
                const itemToAdd = medicalStore.masterCodeList[index];
                if (!itemToAdd) return false;

                // Check if an item with the same dataid AND dbname already exists
                return !medicalStore.shortlistedCodes?.some(
                    shortlistedItem =>
            ...
            .map(value => {
                shortlistedCodes.push(medicalStore.masterCodeList[value]);
                shortlistedIndexes.push(medicalStore.masterCodeList[value]?.dataid);
            });

        medicalStore.addToShortlistedCodes(shortlistedCodes);
        medicalStore.addToShortlistedIndexes(shortlistedIndexes);

        const added = shortlistedCodes.length;
        const skipped = requested - added;

        skipped > 0
            ? ShowSuccess(`Added: ${added} codes. Skipped: ${skipped} code(s).`)
