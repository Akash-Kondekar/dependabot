...
const BaselineSummary = ({ setStep, studyDesign, addOn }) => {
    ...
    const Header = () => (
        ...
        >
            <span>{baselineCharacteristicsTitle}</span>
            {setStep && (
        ...
        <>
            {hasBaseline ? (
                <List sx={{ listStyle: "decimal", pl: 1, marginLeft: "20px" }}>
                    {clinicalCodeListForSummary.map((variable, index) => {
                        return (
                            <ListItem
                                key={variable.label + "_" + index}
                        ...
                        return (
                            <ListItem key={ahd.label} sx={{ display: "list-item" }}>
                                <ListItemText>
                                    ...
                                    {` is defined as `}
                                    <strong>
                                        {baselineVariableTypeOfRecordLabelMap.get(ahd.typeOfRecord)}
                                    </strong>
                                    {ahd.typeOfRecord !== 3 && (
                                        <MarginLabel
