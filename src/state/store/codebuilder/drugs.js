...
import MasterDrug from "../../model/codebuilder/drugs/master-drug";
import DrugBank from "../../model/codebuilder/drugs/drug-bank";
...
export class DrugStore extends BaseStore {
    constructor() {
        super();
        makeObservable(this, {
            setLoading: action,
    ...
    setPreviousResults() {
        const DRUG = CODE_BUILDER_TYPE.DRUG;

        this.setMasterCodeList(
            localStore.get(session.loggedInUser + KEYS.CODES_TO_FILTER + DRUG) ?? []
        );

        this.setShortlistedCodes(
            localStore.get(session.loggedInUser + KEYS.SHORTLISTED_CODES + DRUG) ?? []
        );

        this.setShortlistedIndexes(
            localStore.get(session.loggedInUser + KEYS.SHORTLISTED_INDEXES + DRUG) || []
        );
    }
    ...
    async fetchCodesToCompare() {
        ...
        try {
            ...
            let result = data.data?.listOfCodes?.map(entry => {
                return new MasterDrug(entry);
            });

            this.setFirstCodeBeingCompared(result);

            url = `/drugbank?id=${code2}`;
    ...
    setShortlistedCodes(selectedCode) {
        ...
        localStore.set(
            session.loggedInUser + KEYS.SHORTLISTED_CODES + CODE_BUILDER_TYPE.DRUG,
            this.shortlistedCodes
        );
    }
    ...
    setShortlistedIndexes(value) {
        ...
        localStore.set(
            session.loggedInUser + KEYS.SHORTLISTED_INDEXES + CODE_BUILDER_TYPE.DRUG,
            this.shortlistedIndexes
        );
    }
    ...
    setMasterCodeList(results) {
        ...
        localStore.set(
            session.loggedInUser + KEYS.CODES_TO_FILTER + CODE_BUILDER_TYPE.DRUG,
            this.masterCodeList
        );
    }
    ...
    async upload(codes, database) {
        try {
            ...
            const { data } = await http.post("/drug/handleUpload", payload).stagger(STAGGER_ID);
            ...
            const excludedCodes = new Set([
                ...
                ...result.map(row => row.bnf1),
                ...result.map(row => row.bnf2),
                ...result.map(row => row.bnf3),
                ...result.map(row => row.atc),
                ...result.map(row => row.additionalCode1),
            ]);
    ...
    async delete(code) {
        ...
        try {
            ...
            let { data } = await http.delete(`/drugbank/delete?bankid=${code}`).stagger(STAGGER_ID);
    ...
    async loadUsersRecords() {
        try {
            const url = `/drugbankonly/init?ownerid=${session.loggedInUser}`;

            this.setLoading(true);
            const { data } = await http.get(url).stagger(STAGGER_ID);

            const result = data.data?.map(entry => {
                return new DrugBank(entry);
            });
    ...
    async save(payload) {
        ...
        try {
            ...
            result === "success" ? ShowSuccess("Save successful") : ShowError("Already exists");
            return true;
        } catch (ex) {
            const error = ex.response?.data?.errorDetails;
    ...
    async update(payload, requestType = PHENOTYPE_STATE.DRAFT) {
        try {
            ...
            if (result) {
                ...
                libraryCodeList[target] = new DrugBank(entry);
    ...
    async favoriteCode(id) {
        ...
        this.setLoading(true);
        try {
            const url = `/drugbank/${id}/favorite`;
    ...
    async updateStatus(bankid, status) {
        try {
            ...
            const { data } = await http
                .put(`drugbank/${bankid}/updateStatus/${status}`)
                .stagger(STAGGER_ID);
